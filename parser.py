...
def yargy_parser(path):
    ...
    rule_1 = (
        rule(
            NAME_mtbf,
            (TRESH.optional()).repeatable(),
            X_mtbf).repeatable()).interpretation(RULE)

    rule_2 = (
        rule(
            NAME_mttr,
            (TRESH.optional()).repeatable(),
            X_mttr).repeatable()).interpretation(RULE)
    ...
    n = 10000
    text = [line[i - 5 if i - 5 > 0 else 0:i + n + 5 if i + n + 5 <
                 len(line) else len(line) - 1] for i in range(0, len(line), n)]
    ...
    for link in b:
        for i in range(len(b[link])):
            ...
            if ('years' or 'year' or 'год') in b[link][i].num:
                ...
                # At first we replace ',' to '' and split string for grabbing
                # the number
